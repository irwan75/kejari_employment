...
import 'package:kejari_employment/controller/edit_pegawai_controller.dart';
...
import 'package:kejari_employment/services/update_db.dart';
import 'package:kejari_employment/view/main_list_pegawai.dart';

class EditPegawai extends StatelessWidget {
  ProfilPegawai hasil;
  List<String> listCatMutasi = [];
  EditPegawai({
    Key key,
    this.hasil,
    this.listCatMutasi,
  });
  TextEditingController _formNama;
  TextEditingController _formNip;
  TextEditingController _formPangkatGol;
  TextEditingController _formPangkatTMT;
  TextEditingController _formJabatanNama;
  TextEditingController _formJabatanTMT;
  TextEditingController _formMasKerTahun;
  TextEditingController _formMasKerBulan;
  TextEditingController _formLatJabNama;
  TextEditingController _formLatJabWktu;
  TextEditingController _formLatJabJmlJam;
  TextEditingController _formPendidikanNama;
  TextEditingController _formPendidikanThn;
  TextEditingController _formPendidikanIjazah;
  TextEditingController _formUsia;
  TextEditingController _formMutasiKepegawaian = TextEditingController();

  final EditPegawaiController _editPegawaiController =
      Get.put(EditPegawaiController());
  ...
  Widget build(BuildContext context) {
    _formNama = TextEditingController(text: hasil.nama);
    _formNip = TextEditingController(text: "${hasil.nip}");
    _formPangkatGol = TextEditingController(text: hasil.pangkatGol);
    _formPangkatTMT = TextEditingController(text: hasil.pangkatTmt);
    _formJabatanNama = TextEditingController(text: hasil.jabNama);
    _formJabatanTMT = TextEditingController(text: hasil.jabTmt);
    _formMasKerTahun = TextEditingController(text: "${hasil.maskerThn}");
    _formMasKerBulan = TextEditingController(text: "${hasil.maskerBln}");
    _formLatJabNama = TextEditingController(text: hasil.latjaNama);
    _formLatJabWktu = TextEditingController(text: hasil.latjaWkt);
    _formLatJabJmlJam = TextEditingController(text: "${hasil.latjaJmlJam}");
    _formPendidikanNama = TextEditingController(text: hasil.pendidikanNama);
    _formPendidikanThn =
        TextEditingController(text: "${hasil.pendidikanThnLls}");
    _formPendidikanIjazah = TextEditingController(text: hasil.pendidikanIjazah);
    _formUsia = TextEditingController(text: "${hasil.usia}");
    _image = File(hasil.pathPicture);
    return Scaffold(
      appBar: AppBar(
        title: Text("Edit Data ${hasil.nama}"),
        centerTitle: true,
      ...
      body: Container(
        ...
        child: ListView(
          children: [
            SizedBox(height: 20),
            GetBuilder<EditPegawaiController>(
              id: 'profilPicture',
              init: EditPegawaiController(),
              builder: (controller) {
            ...
            MaterialButton(
              ...
              onPressed: () async {
                if (_formNama.text.isEmpty ||
                  ...
                  try {
                    var result = await UpdateDb().updateEmployee(
                        new ProfilPegawai(
                          id: hasil.id,
                          ...
                          maskerThn: int.parse(_formMasKerBulan.text),
                          nama: _formNama.text,
  ...
  Widget addCatMutasi() {
    return GetBuilder<EditPegawaiController>(
        ...
        return Column(
          ...
          children: [
            ...
            ListView.builder(
              ...
              itemCount: listCatMutasi.length,
              itemBuilder: (context, index) {
                return GestureDetector(
                  onTap: () {
                    listCatMutasi.removeAt(index);
  ...
  Widget buttonCircleCatMutasi(IconData icon, Color color, int kondisi) {
    return GestureDetector(
      onTap: () {
        if (kondisi == 0) {
          _editPegawaiController.setMutasiKepegawaian(0);
        } else {
          print(_formMutasiKepegawaian.text);
          listCatMutasi.add(_formMutasiKepegawaian.text);
  ...
  Widget listTileCustom(
      String title, TextEditingController formText, TextInputType type) {
    return Container(
  ...
  Widget rowKeteranganEdit(TextEditingController formText, TextInputType type) {
    return Container(
      child: Expanded(
        flex: 1,
        child: TextFormField(
